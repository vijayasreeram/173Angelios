...
object ColloquialEngine {
    ...
    private fun formalHindi(text: String): String {
        ...
        val phraseMap = listOf(
            ...
            Pair(Regex("कहाँ हो ভাই\\??"), "आप कहाँ हैं?"),
    ...
    private fun colloquialTamil(text: String, isCasualSrc: Boolean): String {
        var t = text
        val phraseMap = listOf(
            Pair(Regex("வணக்கம்,\\s*நீங்கள் எப்படி இருக்கிறீர்கள்\\?"), "ஹாய், எப்படி இருக்கீங்க?"),
            ...
            Pair(Regex("மன்னிக்கவும்"), "சாரி"),
            Pair(Regex("தயவுசெய்து"), "ப்ளீஸ்"),
            Pair(Regex("மிக்க நன்றி"), "ரொம்ப தேங்க்ஸ்"),
            Pair(Regex("நன்றி"), "தேங்க்ஸ்"),
        ...
        if (isCasualSrc) {
            t = Regex("^" + endWb("வணக்கம்").pattern).replace(t, "ஹாய்")
        }
